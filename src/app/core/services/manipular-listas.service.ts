...
export class ManipularListasService {
  ...
  // Método para obtener datos de JSONPlaceholder, mapearlos y almacenarlos en localStorage
  fetchData(): Observable<MappedPost[]> {
    ...
    return this.http.get<Post[]>(this.apiUrl).pipe(
      ...
      catchError((error) => {
        ...
        return throwError(() => new Error('Error al obtener los datos')); // Propaga el error
      })
    );
  }
  ...
  // Método para limpiar los datos del localStorage
